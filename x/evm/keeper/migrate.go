...
func GetMigrationHandler(k types.BaseKeeper, n types.Nexus) func(ctx sdk.Context) error {
	return func(ctx sdk.Context) error {
		for _, chain := range n.GetChains(ctx) {
			...
			deleteTokenDeploymentCommand(ctx, ck, token)
			deleteTokenBurners(ctx, ck, token)
			migrateConfirmedDepositsToBurnt(ctx, ck, token)
			n.DeregisterAsset(ctx, chain, token.Asset)
		}
...
func deleteTokenBurners(ctx sdk.Context, ck chainKeeper, token types.ERC20TokenMetadata) {
	...
	for ; iter.Valid(); iter.Next() {
		...
		if !bytes.Equal(burner.TokenAddress.Bytes(), token.TokenAddress.Bytes()) {
			continue
		}
...
func migrateConfirmedDepositsToBurnt(ctx sdk.Context, ck chainKeeper, token types.ERC20TokenMetadata) {
